import { DomEvents, Vector2, WebKitGestureEvent } from '../types'
...
export function getTwoTouchesEventValues(
  ...
  const [A, B] = Array.from(event.touches).filter((t) =>
    pointerIds.includes(t.identifier),
  )
 ...
 */
export function getScrollEventValues(
  event: UIEvent,
  transform = identity,
): Vector2 {
 ...
 */
export function getWheelEventValues(
  event: WheelEvent,
  transform = identity,
): Vector2 {
 ...
 */
export function getWebkitGestureEventValues(
